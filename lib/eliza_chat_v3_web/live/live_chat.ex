defmodule ElizaChatV3Web.LiveChat do
  ...
  def render(assigns) do
    initial_prompts_to_skip = 2
    filtered_history = Enum.drop(assigns.history, initial_prompts_to_skip)
    assigns = assign(assigns, :filtered_history, filtered_history)
    ~H"""
    ...
    <div class="message-container p-4 rounded-md shadow-md">
      <%= for {label, message} <- Enum.zip(Stream.cycle(["Me", "Ciku"]), @filtered_history) do %>
